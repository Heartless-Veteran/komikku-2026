...
class ReadingTimeEstimator(
    ...
    fun getAverageReadingSpeed(mangaId: Long): Flow<Float?> {
        return flow {
            val historyList = getHistory.await(mangaId)
            emit(calculateReadingSpeed(historyList))
        }
    }
    ...
    private fun calculateReadingSpeed(historyList: List<History>): Float? {
        val validEntries = historyList.filter { it.readDuration > 0 && it.chapterId > 0 }
        if (validEntries.size < 2) return null // Need at least 2 data points

        val totalTime = validEntries.sumOf { it.readDuration }
